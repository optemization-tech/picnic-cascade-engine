...
**What the button does:** After carryover + inception, wires **Production Task** (alias **Notion Task**) relations on Migrated Tasks, applies completion overlay (max **Date Completed** across contributors per cascade task, skipping **Manual Workstream / Item** where applicable), assigns owners when assignee text resolves to a workspace user, writes **Match Confidence** (`High` / `Medium` / `Low`) on each linked Migrated Task — the cascade-side `Match Confidence` is a rollup through `Notion Task` ↔ `Asana Task`, so this populates both surfaces — **cleans up the source row's title and page icon** by stripping the study-name prefix from the title and lifting any leading emoji marker (`🔶 ✅ 🔷 ⚠️ 🚨`) into the page icon (e.g., `🔶  Alexion PNH PLEDGE Final SAP Delivery` becomes title `Final SAP Delivery` + icon `🔶`), sets **Migration Status**, and marks remaining unmatched cascade tasks **Blueprint-default**. The title and icon writes only fire on matched rows where the cleanup actually changes something — idempotent on re-runs, and unmatched rows keep their original title and icon for PM context.
...
- `medium` (matcher normalized / paren-strip / milestone-fallback / milestone-inferred) → **Medium**
