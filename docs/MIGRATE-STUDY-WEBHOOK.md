...
**What the button does:** After carryover + inception, wires **Production Task** (alias **Notion Task**) relations on Migrated Tasks, applies completion overlay (max **Date Completed** across contributors per cascade task, skipping **Manual Workstream / Item** where applicable), assigns owners when assignee text resolves to a workspace user, writes **Match Confidence** (`High` / `Medium` / `Low`) on each linked Migrated Task — the cascade-side `Match Confidence` is a rollup through `Notion Task` ↔ `Asana Task`, so this populates both surfaces — **renames the source title** to drop the study-name prefix while preserving any leading emoji marker (e.g., `🔶  Alexion PNH PLEDGE Final SAP Delivery` → `🔶 Final SAP Delivery`), sets **Migration Status**, and marks remaining unmatched cascade tasks **Blueprint-default**. The title rename only fires on matched rows whose cleaned title differs from the original — idempotent on re-runs and unmatched rows keep their original title for PM context.
